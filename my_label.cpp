...
void my_label::mousePressEvent(QMouseEvent *mouse_event)
{
    ...
    {   paint_seed_flag=false;
        ...
        if(seed_pos.x()<=this->size().width() && seed_pos.y()<=this->size().height())
        {
            ...
            {
                //emit sendSeedPosition(seed_pos);
...
double* my_label::compute_cost_link(QImage &img, int i, int j)
{
     ...
     double BDlink[8];
     uint RawColor[8];
     if((i+1)<img_width)RawColor[0]=img.pixel(i+1,j);
     if((i+1)<img_width&&(j-1)>=0)RawColor[1]=img.pixel(i+1,j-1);
     if((j-1)>=0)RawColor[2]=img.pixel(i,j-1);
     if((i-1)>=0&&(j-1)>=0)RawColor[3]=img.pixel(i-1,j-1);
     if((i-1)>=0)RawColor[4]=img.pixel(i-1,j);
     if((i-1)>=0&&(j+1)<img_height)RawColor[5]=img.pixel(i-1,j+1);
     if((j+1)<img_height)RawColor[6]=img.pixel(i,j+1);
     if((i+1)<img_width&&(j+1)<img_height)RawColor[7]=img.pixel(i+1,j+1);
     for(int k=0;k<8;k++)
     {
...
void my_label::recover_matrix()
{
    for(int i=0;i<img_width;i++)
    {
        ...
        {
            int index=j*img_width+i;
            node_graph[index]->prevNode=NULL;
            node_graph[index]->totalCost=max_weight;
